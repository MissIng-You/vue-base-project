  ...
  <div id="add-view">
    <modal id="deleteUserModal" size="sm" :fade="true">
      ...
      <div slot="modal-body">
        ...
        <span>确认删除【{{meta.UserName}}】记录吗?</span>
      </div>
      <div slot="modal-footer">
        <div class="label label-vertical label-info label-pill">{{message}}</div>
        <div class="pull-right">
          <button type="button" @click="onDeleteUser" class="btn btn-danger btn-xs" ><i class="fa fa-fw fa-lg fa-trash"></i>删除</button>
    ...
    methods: {
      _hideModal () {
        ...
        window.setTimeout(function () {
          self.$broadcast('hide::modal', 'deleteUserModal')
          self.$set('message', '')
        }, 1000)
      },
      onDeleteUser () {
